...
var elements = {
	...
	username: document.getElementById("tweet--info__names--username"),
	verified: document.getElementById("tweet--info__names--verified"),
	profilePicture: document.getElementById("tweet--profile-picture"),
	tweet: document.getElementById("tweet--text"),
	date: document.getElementById("tweet--details"),

	quoteContainer: document.getElementById("tweet--quote-tweet__container"),
	quoteFullname: document.getElementById("tweet--quote-tweet__names--name"),
	quoteUsername: document.getElementById("tweet--quote-tweet__names--username"),
	quoteTweet: document.getElementById("tweet--quote-tweet__text"),
	quoteVerified: document.getElementById("tweet--quote__names--verified"),

	inputProfilePicture: document.getElementById("options--profile-picture-upload"),
	inputShowReply: document.getElementById("header--options__enable-reply"),
	inputVerified: document.getElementById("header--options__user-verified"),
	inputReplyVerified: document.getElementById("header--options__reply-user-verified")
}
...
elements.inputShowReply.onchange = event => {
	if (event.target.checked) {
		elements.quoteContainer.style.display = "block"
	} else {
		elements.quoteContainer.style.display = "none"
	}
}

elements.inputVerified.onchange = event => {
	if (event.target.checked) {
		elements.verified.style.display = "block"
	} else {
		elements.verified.style.display = "none"
	}
}

elements.inputReplyVerified.onchange = event => {
	if (event.target.checked) {
		elements.quoteVerified.style.display = "block"
	} else {
		elements.quoteVerified.style.display = "none"
	}
}

elements.inputProfilePicture.onchange = function(e) {
	var files = e.target.files
	var fileReader = new FileReader()

	fileReader.onload = function(reader) {
		console.log(reader)
		elements.profilePicture.style.backgroundImage = "url('" + reader.target.result + "')"
	}
...
var saveButton = document.getElementById("header--options__tips--save")
var tweet = document.getElementById("tweet--container")

saveButton.onclick = function(e) {
		var canvas = document.createElement("canvas")
		var scaleBy = 5
		var width = tweet.offsetWidth
		var height = tweet.offsetHeight
